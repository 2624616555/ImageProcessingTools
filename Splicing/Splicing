...
def image_coor(image, w_num, h_num, size):
    ...
    for i in range(w_num):
        if i == (w_num - 1):
            start_list_w.append(w-size)
            end_list_w.append(w)
        else:
            start_list_w.append((size - overlap_w) * i)
            end_list_w.append((size - overlap_w) * i + size)
    for i in range(h_num):
        if i == (h_num - 1):
            start_list_h.append(h-size)
            end_list_h.append(h)
        else:
            start_list_h.append((size - overlap_h) * i)
            end_list_h.append((size - overlap_h) * i + size)
...
def splicing_image(image_patch_list,coor):
    ...
    sava_image = np.zeros((end_list_w[-1],end_list_h[-1],3))
